...
type_sequence = ['Middlebury/']
directories_images = ['../data/Middlebury']
...
for idx_directory in range(len(directories_images)):
	...
	for i in range(len(folders_sequences)):
		...
		for j in {3}:
			...
			f1 = cut(delete(confi(im_name0, im_name1, match_name_1, f_path), threshold))
			print f1
			#I1-I0
			...
			print command_line
			#os.system(command_line)
			f2 = cut(delete(confi(im_name1, im_name0, match_name_2, f_path), threshold))
			print f2
