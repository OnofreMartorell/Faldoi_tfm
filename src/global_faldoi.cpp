    ...
    )
{
  ...
  float *u1x    = (float *) xmalloc(size*sd);
  float *u1y    = (float *) xmalloc(size*sd);
  float *u2x    = (float *) xmalloc(size*sd);
  float *u2y    = (float *) xmalloc(size*sd);

  float *v1     = (float *) xmalloc(size*sd);
  float *v2     = (float *) xmalloc(size*sd);

  float *rho_c  = (float *) xmalloc(size*sd);
  float *grad  = (float *) xmalloc(size*sd);

  float *u1_     = (float *) xmalloc(size*sd);
  float *u2_     = (float *) xmalloc(size*sd);

  float *u1Aux   = (float *) xmalloc(size*sd);
  float *u2Aux   = (float *) xmalloc(size*sd);

  float *I1x    = (float *) xmalloc(size*sd);
  float *I1y    = (float *) xmalloc(size*sd);

  float *I1w    = (float *) xmalloc(size*sd);
  float *I1wx   = (float *) xmalloc(size*sd);
  float *I1wy   = (float *) xmalloc(size*sd);

  //Divergence
  float *div_xi1 = (float *) xmalloc(size*sd);
  float *div_xi2 = (float *) xmalloc(size*sd);
  
  float *u_N = (float *) xmalloc(size*sd);
...
static inline bool positive(int val)
{
  return val >= 0;
}
...
static float get_wspatial( int l, int k)
{
  float ws = MAX_BETA;
  float w_tmp;
  float difS = 0.0;
  ...
  )
{
  ...
  DualVariables *p = new DualVariables[size];
  DualVariables *q = new DualVariables[size];
...
//#endif//DISABLE_OMP
  ...
  // //Median filter before anything
  // for (int j = 2; j < h[0]-2; j++){
  //   for(int i = 2; i < w[0]-2; i++){
  //     u[j*w[0] + i] = med_filter3x3_at(u, j, i, w[0], h[0]);
  //     v[j*w[0] + i] = med_filter3x3_at(v, j, i, w[0], h[0]);
  //   }
  // }
