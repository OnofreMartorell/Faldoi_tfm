...
void match_growing_variational(
  ...
  ofGo.u2  = ofGo.u1 + w*h;
  ofGo.fixed_points = new int[w*h];
  ofGo.trust_points = new int[w*h];
  ofGo.saliency = sal_go;
  ofGo.wr = w_radio;
  ofGo.w = w;
  ofGo.h = h;
  ofGo.method = method;
  //TODO: Only to keep if it presents better performance.
  ofGo.weight = new BilateralWeight[w*h]; // weight of non local
  ofGo.u1_ini = new float[(2*w_radio + 1)*(2*w_radio + 1)];
  ofGo.u2_ini = new float[(2*w_radio + 1)*(2*w_radio + 1)];
  ...
  ofBa.u2  = ofBa.u1 + w*h;
  ofBa.fixed_points = new int[w*h];
  ofBa.trust_points = new int[w*h];
  ofBa.saliency = sal_ba;
  ofBa.wr = w_radio;
  ofBa.w = w;
  ofBa.h = h;
  ofBa.method = method;
  //TODO: Only to keep if it presents better performance.
  ofBa.weight = new BilateralWeight[w*h]; // weight of non local
  ofBa.u1_ini = new float[(2*w_radio + 1)*(2*w_radio + 1)];
  ofBa.u2_ini = new float[(2*w_radio + 1)*(2*w_radio + 1)];
  // rand_local_patch_ini(ofBa.u1_ini,w_radio);
  // rand_local_patch_ini(ofBa.u2_ini,w_radio);
  // zero_local_patch_ini(ofBa.u1_ini,w_radio);
  // zero_local_patch_ini(ofBa.u2_ini,w_radio);
...
//                          0      1  2  3   4      5         6       7
    ...
    fprintf(stderr, "usage %d :\n\t%s i0 i1 in0.flo in1.flo out.flo sim_map.tiff sal0.tiff sal1.tiff"
//                          0      1  2  3   4      5         6       7
                    "[-m method_id] [-wr windows_radio]\n",c, *v);
  ...
  char *filename_go  = v[3];
  char *filename_ba  = v[4];
  char *filename_out = v[5];
  char *filenme_sim  = v[6];
  char *filename_sal0;
  char *filename_sal1;
